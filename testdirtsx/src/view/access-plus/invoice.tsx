...
export default function AccessPlusInvoiceCollectionView(props: AppModel) {
    ...
    const RESOURCE_NAME = "accessPlus/invoices";
    ...
    React.useEffect(() => {
        ...
        grid.jqGrid('setGridParam', {
            data: arr.map(fm => { return { 'id': fm.id, 'cell': [...meta.map(x => (fm as any)[x.name]), fm.subgrid] } })
        });
    ...
    return <div>
        ...
        <button
            ...
            onClick={e => {
                props.setIsLoading(true);
                InvoiceAPI.createPaymentsForAccessPlusInvoices()
                    .then(r => {
                        if (r && r.length != 0)
